...
const getAll: Handler = async (_req, res) => {
	...
	try {
		const fileNames = await fsp.readdir(
			resolve(config.SCHEMAS_DIR, 'singles')
		);
		...
		schemas = await Promise.all(
			fileNames.map(async fileName => (
				JSON.parse(
					await fsp.readFile(
						resolve(config.SCHEMAS_DIR, `singles/${fileName}`),
						'utf-8'
					)
				) as SingleSchema
			))
...
const get: Handler = async (req, res) => {
	...
	try {
		schema = JSON.parse(
			await fsp.readFile(
				resolve(config.SCHEMAS_DIR, `singles/${name}.json`),
				'utf-8'
			)
		) as SingleSchema;
...
const post: Handler = async (req, res) => {
	...
	try {
		if (
			fs.existsSync(
				resolve(config.SCHEMAS_DIR, `singles/${schema.name}.json`)
			)
		) {
...
export const privateSinglesSchemaController = { getAll, get, post, patch, delete: _delete };
